...
copy_vivado_outputs:
	mkdir -p ./bin
	if [ -f vivado/vivado.runs/impl_1/top.bit ]; then cp vivado/vivado.runs/impl_1/top.bit ./bin/top_$(shell date +%m%d)_1.bit; fi
	if [ -f vivado/vivado.gen/sources_1/bd/ps_interface/hw_handoff/ps_interface.hwh ]; then cp vivado/vivado.gen/sources_1/bd/ps_interface/hw_handoff/ps_interface.hwh ./bin/top_$(shell date +%m%d)_1.hwh; fi
